...
void main() {
    ...
    vec2 ct = (window.projection * window.view * vec4(center.xy, 0.0, 1.0)).xy;
    ...
    vec3 pos = (window.view * vec4(center.xyz, 1.0)).xyz;
    ...
    gs_color = v_color[0];
    // Upper left
    gl_Position = mvp * vec4(rot * vec2(-hsize.x, hsize.y) + pos.xy, pos.z, 1.0);
    gs_uv = uv0;
    EmitVertex();
